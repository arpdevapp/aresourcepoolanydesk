...
class WebrtcService : Service(), MainRepository.Listener {
    ...
    override fun onCreate() {
        ...
        // Always start as foreground service to support media projection
        ...
        if (intent != null) {
            when (intent.action) {
                ...
                "AcceptCallIntent" -> {
                    ...
                    target?.let { mainRepository.startCall(it) }
                }

                "RequestConnectionIntent" -> {
                    val target = intent.getStringExtra("target")
                    target?.let {
                        screenPermissionIntent?.let { permissionIntent ->
                            surfaceView?.let { surface ->
                                mainRepository.setPermissionIntentToWebrtcClient(permissionIntent)
                                mainRepository.startScreenCapturing(surface)
                                mainRepository.sendScreenShareConnection(it)
                            } ?: run {
                                android.util.Log.e("WebrtcService", "surfaceView is null, cannot start screen sharing")
                            }
                        } ?: run {
                            android.util.Log.e("WebrtcService", "screenPermissionIntent is null, cannot start screen sharing")
                        }
                    }
                }
            }
    ...
    private fun startServiceWithNotification() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            val notification = NotificationCompat.Builder(this, "channel1")
                ...
                .setContentTitle("WebRTC Screen Share")
                .setContentText("Screen sharing service is active")
